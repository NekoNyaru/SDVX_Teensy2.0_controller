...
//▼ 부품
  ...
  #define stripLED1 36    // 스타트버튼 LED
  #define stripLED2 22    // 버튼 위 LED
//▲
...
//▼ 핀 선언
  ...
  #define FVRIT 11
  //추가 LED 선언
  #define LED1 15     //스타트 LED
  #define LED2 14     //버튼 위 LED
...
//▼ LED 관련 구문
  ...
  Adafruit_NeoPixel strip3 = Adafruit_NeoPixel(stripLED1, LED1, NEO_RGB + NEO_KHZ800);
  Adafruit_NeoPixel strip4 = Adafruit_NeoPixel(stripLED2, LED2, NEO_RGB + NEO_KHZ800);
  ...
  for (int ch = 0; ch < num_pins; ch++) {
    ...
    for (int ch = 1; ch <= 4; ch++) { // BT 1 ~ 4
      ...
      if (b_stat[ch] == true) {
        ...
        for (int i = BTN_S[chm]; i <= BTN_E[chm]; i++) {  //
          strip4.setPixelColor(i, 255, 255, 255);            //각 버튼 위에 달린 LED 점등 관련
        }
    ...
    for (int ch = 5; ch < 7; ch++) { //FX-L , R
      ...
      if (b_stat[ch] == true) {
        ...
        for (int i = BTN_S[chm]; i <= BTN_E[chm]; i++) {  //
          strip4.setPixelColor(i, 0, 255, 0);              //각 버튼 위에 달린 LED 점등 관련
        }
        strip4.setBrightness(255);
        strip4.show();
      } else {
...
//-------------------------------------------------------------------------------// 노브 작동, 노브 LED
    ...
    if ((runningtime - novllastmovetime) >= novledofftime) { //일정시간 작동안할시 L스트립LED 위치 초기화
      strip1.clear();
      strip1.show();
      novro_l = 350.0;
      clearL = true;
    ...
    if ((runningtime - novrlastmovetime) >= novledofftime) { //일정시간 작동안할시 R스트립LED 위치 초기화
      strip2.clear();
      strip2.show();
      novro_r = 360.00;
      clearR = true;
    ...
    if (clearL && clearR) { //둘다 작동 안 할 떄 스트립 LED OFF
     strip3.clear();
     strip3.show();
    }
 ...
 /*
  ...
  (L코드) : setPosition(strip1, ledon1, strip1.Color(움직이는 LED의 중앙 색깔), strip1.Color(빨강, 초록, 파랑), ((uint32_t)빨강 << 16) | ((uint32_t)초록 << 8) | (uint32_t)파랑, novspd_l < 0 ? -2 : 2);
            setPositionS(strip3, ledon3, strip1.Color(움직이는 LED의 중앙 색깔), strip3.Color(초록, 빨강, 파랑));
  ...
  (R코드) : setPosition(strip2, ledon2, strip2.Color(움직이는 LED의 중앙 색깔), strip2.Color(빨강, 초록, 파랑), ((uint32_t)빨강 << 16) | ((uint32_t)초록 << 8) | (uint32_t)파랑, novspd_r < 0 ? -2 : 2);
            setPositionS(strip3, ledon4, strip2.Color(움직이는 LED의 중앙 색깔), strip3.Color(초록, 빨강, 파랑));
  ...
  제가 사용한 스트립LED는 GRB세팅이라 초록 빨강 파랑 순서인데, 기본값은 RGB세팅, 즉 빨강 초록 파랑 순서입니다 상황에 맞게 스왑해서 사용하세요
 */
...
void setPositionS(Adafruit_NeoPixel &strip, int ledon, uint32_t mainColor, uint32_t adjacentColor) {
  for (int i = 0; i < ledlight; i++) {
    int pixelIndex = ledon - (i * (stripLED0 / ledlight));
    strip.setPixelColor(pixelIndex, mainColor);
